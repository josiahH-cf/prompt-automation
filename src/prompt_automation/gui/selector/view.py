"""Tkinter view and callbacks for the template selector GUI.
...
def _manage_overrides(root: tk.Tk, service):
...
def open_template_selector(service) -> Optional[dict]:
    ...
    browser = service.create_browser_state()
    browser.build()
    ...
    def _on_digit(event):
        if event.char and event.char.isdigit():
            tmpl = service.resolve_shortcut(event.char)
            if tmpl:
...
def _manage_shortcuts(root: tk.Tk, service):  # pragma: no cover - GUI only
    win = tk.Toplevel(root)
    ...
    mapping = service.load_shortcuts()  # key -> rel path
    ...
    def _edit_key(event=None):
        ...
        def _apply(new_key: str):
            ...
            if new_key:
                mapping[new_key] = rel
            service.save_shortcuts(mapping)
            _refresh_tree()
    ...
    def _do_delete():
        ...
        for k in list(mapping.keys()):
            if mapping[k] == rel:
                mapping.pop(k, None)
        service.save_shortcuts(mapping); _refresh_tree()
    def _do_renumber():
        # Apply renumbering according to numeric shortcut keys
        try:
            service.renumber_templates(mapping)
